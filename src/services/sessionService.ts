...
// Validates a live session WITHOUT rotating the refresh token, and returns its
// identity so a fresh access token can be minted. Used by the silent-renew
// endpoint: products call it on 401, and because it doesn't rotate, several
// products/tabs renewing at once can't trip reuse-detection and log the user out.
export async function validateSession(refreshToken: string): Promise<ValidatedSession | null> {
  const session = await findByToken(refreshToken);
  if (!session || session.revoked_at || new Date(session.expires_at) < new Date()) {
    return null;
  }
  ...
  await pool
    .request()
    .input("id", sql.UniqueIdentifier, session.id)
    .query(`UPDATE dbo.login_session SET last_used_at = SYSUTCDATETIME() WHERE id = @id`);
  return {
